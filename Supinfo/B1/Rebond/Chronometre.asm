...
;  |||   gestion du chronometre   |||  
     chronometre PROC   
          ...
          PUSH BX       ; on empile BX
          PUSH CX       ; on empile CX
          PUSH DX       ; on empile DX
          
          MOV AX, DX    ; on stocke DX dans AX
          MOV BX, 10    ; on stocke 10 dans BX
          MOV CX, 0     ; on stocke 0 dans CX
          
 
          loop_A: 
            MOV DX, 0   ; on stocke 0 dans DX
            DIV BX      ; on divise par BX
            PUSH DX     ; on stocke DX
            INC CX      ; on incremente CX
            
            CMP AX, 0   ; on compare AX a 0
            JA loop_A   ; si superieur on recommence
            MOV BX, 18  ; on met 18 dans BX
          
          loop_B:
            POP AX      ; on depile AX 
            ADD AX, 48  ; on ajoute 48 a AX
            MOV [BX], Al; on met Al dans [BX]
            ADD BX, 2   ; on ajoute 2 a BX
            
          loop loop_B   ; on boucle     
...
; generation affichage chronometre 
 MOV [02], 'C'
 MOV [04], 'h'
 MOV [06], 'r'
 MOV [08], 'o'
...
;  |||  gestion du deplacement   |||                          
  ...
  boucle2:          ; boucle de direction en bas a droite            
    ...
    INC DX                  ; on incremente DX
        CALL chronometre    ; on appelle la procedure pour le chronometre
